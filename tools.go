// Package mcpholded provides generic MCP tool helpers: a reflection-based
// schema generator and the MustTool/ConvertTool factory that wires Go handler
// funcs into the mcp-go server.
//
// This file lives at the module root (same directory as go.mod). Because Go
// package names cannot contain hyphens, mcpholded is the hyphen-free form
// of github.com/luisra51/mcp-holded (e.g. module "mcp-acme" → package "mcpacme").
package mcpholded
...
// MustTool wraps ConvertTool and panics on schema error. Panic at init time is
// intentional — it surfaces schema bugs before the process starts serving.
...
func ConvertTool[T any, R any](name, description string, toolHandler ToolHandlerFunc[T, R], options ...mcp.ToolOption) (mcp.Tool, server.ToolHandlerFunc, error) {
	...
	handler := func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		config := holded.ConfigFromContext(ctx)
		ctx, span := otel.Tracer("github.com/luisra51/mcp-holded").Start(ctx, fmt.Sprintf("mcp.tool.%s", name))
		defer span.End()
		span.SetAttributes(
